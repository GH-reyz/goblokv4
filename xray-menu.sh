...
menu_sts() {
    chck_pid
    if [[ ! -z "${PID}" ]]; then
        echo -e "Status  : ${Green_font_prefix} Installed [✓]${Font_color_suffix} | ${Green_font_prefix}Running [✓]${Font_color_suffix}"
    else
        echo -e "Status  : ${Green_font_prefix} Installed [✓] ${Font_color_suffix} | ${Red_font_prefix}Not Running [❌] ${Font_color_suffix}"
    fi
}
clear
echo -e ""
